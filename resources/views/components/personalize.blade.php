<div class="modal fade" id="personalizeModal" tabindex="-1" aria-labelledby="personalizeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-8">
                            <h3 class="bo-hoofdkleur">1x Krat (30 stuks)</h3>
                            <p class="no-bottom-margin"><strong>Let op!</strong> </p>
                            <p>Het aanpassen van de inhoud kan invloed hebben op de prijs en moet daarom een goedkeuring krijgen van Vitamine Bo.</p>
                            <p>Personaliseer hier uw krat:</p>
                        </div>
                        <div class="col-md-4">
                            <img src="images/fruitkistbo.png" alt="Image" class="container-fluid pb-2">
                        </div>
                    </div>
                        <div class="row">
                        <div class="col-md-6">
                            <ul class="custom-ul stripe-top">
                            <li>
                                <span class="list-text">Appel</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                            </li>
                            <li>
                                <span class="list-text">Peer</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                            </li>
                            <li>
                                <span class="list-text">Banaan</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                            </li>
                            <li>
                                <span class="list-text">Sinaasappel</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                            </li>
                            <li>
                                <span class="list-text">Kiwi</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                            </li>
                                <li>
                                <span class="list-text">Avocado</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                                </li>
                        </div>

                        <div class="col-md-6">


                            <ul class="custom-ul stripe-top">
                                <li>
                                    <span class="list-text">Mandarijn</span>
                                    <div class="wrapper">
                                        <span class="minus">-</span>
                                        <span class="num">0</span>
                                        <span class="plus">+</span>
                                    </div>
                                </li>
                                <li>
                                    <span class="list-text">Citroen</span>
                                    <div class="wrapper">
                                        <span class="minus">-</span>
                                        <span class="num">0</span>
                                        <span class="plus">+</span>
                                    </div>
                                </li>
                                <li>
                                    <span class="list-text">Druiven</span>
                                    <div class="wrapper">
                                        <span class="minus">-</span>
                                        <span class="num">0</span>
                                        <span class="plus">+</span>
                                    </div>
                                </li>
                                <li>
                                    <span class="list-text">Tomaat</span>
                                    <div class="wrapper">
                                        <span class="minus">-</span>
                                        <span class="num">0</span>
                                        <span class="plus">+</span>
                                    </div>
                                </li>
                                <li>
                                    <span class="list-text">Komkommer</span>
                                    <div class="wrapper">
                                        <span class="minus">-</span>
                                        <span class="num">0</span>
                                        <span class="plus">+</span>
                                    </div>
                                </li>
                                <li>
                                <span class="list-text">Paprika</span>
                                <div class="wrapper">
                                    <span class="minus">-</span>
                                    <span class="num">0</span>
                                    <span class="plus">+</span>
                                </div>
                                </li>
                            </ul>
                            <p>Total: <span id="total-counter">0 / 30</span></p>
                        </div>
                        </div>
                </div>
            </div>
            <div class="modal-footer">
                <div class="d-flex">
                    <x-buttonicon class="modal-trigger me-2" data-bs-toggle="modal" data-bs-target="#exampleModal">
                        Annuleren
                    </x-buttonicon>
                    <x-buttonicon data-bs-dismiss="modal" aria-label="Close" class="ml-2">
                        Wijziging toevoegen
                    </x-buttonicon>
                </div>
            </div>
        </div>
    </div>
</div>
